...
public class DrawHud {
	...
	public DrawHud() {
		try {
			hudtexture = (Hearthstone.rescaledBufferedimage(ImageIO.read(Hearthstone.allImportedFiles[5]),
					(Hearthstone.BREITE < 1920) ? 335 : 446, (Hearthstone.BREITE < 1920) ? 175 : 211));

			// Testing went wrong
			// hudtexture =
			// DrawDeck.rescaledBufferedimage((ImageIO.read(Hearthstone.allImportedFiles[5])),
			// 300, 170);
		} catch (IOException e) {
			hudtexture = null;
			e.printStackTrace();
		...
		hud = new Rectangle(50,
				...
				hudtexture.getWidth(), hudtexture.getHeight());
	}
	 ...
	 * 
	 * @param playersMove
	 *            if the hud is rendered in the players move or not
	 * @param cardInDetail
	 *            the card of which stats get shown on player hud
	 * @param gameStats
	 *            Stats Array that is used to display games current stats
	 * @param g
	 *            the component on that the graphics get drawn
	 */
	public void render(boolean playersMove, Karte cardInDetail, int[] gameStats, Graphics g) {
		// hud texture
		g.drawImage(hudtexture, (int) hud.getX(), (int) hud.getY(), (int) hud.getWidth(), (int) hud.getHeight(), null);
		...
		if ((cardInDetail != null) && cardInDetail.getStatus() != Status.ABBLAGE) {
			...
			// lifeDisplay Player
			String dmgCard = String.format("%2d%5s%-25s", cardInDetail.getSchaden(), "",
					"Schaden von " + cardInDetail.getName());

			g.drawString(dmgCard, (int) (hud.getX() + ((Hearthstone.BREITE < 1920) ? 18 : 25)),
					(int) (hud.getY() + ((Hearthstone.BREITE < 1920) ? 52 : 62)));

			// manaDisplay Player
			String lifeCard = String.format("%2d%5s%-25s", cardInDetail.getLeben(), "",
					"Leben von " + cardInDetail.getName());

			g.drawString(lifeCard, (int) hud.getX() + ((Hearthstone.BREITE < 1920) ? 18 : 25),
	 ...
	 * 
	 * @param gameStats
	 *            these stats are used fpr display
	 * @param g
	...
	public void drawDeckInfo(int[] gameStats, Graphics g) {
		...
		g.drawString("" + gameStats[6], 20, (int) Hearthstone.HOEHE - ((Hearthstone.BREITE < 1920) ? 35 : 10));

		// Abblage Player
		g.drawString("" + gameStats[7], (int) Hearthstone.BREITE - 25,
				(int) Hearthstone.HOEHE - ((Hearthstone.BREITE < 1920) ? 35 : 10));

		// Stapel PC
		g.setColor(Color.red);
		g.drawString("" + gameStats[8], 15, 15);

		// Abblage PC
		g.drawString("" + gameStats[9], (int) Hearthstone.BREITE - 25, 15);
	}
	...
	public void drawInfo(int[] gameStats, Graphics g) {
		g.setColor(new Color(160, 151, 16)); // possibly change this to RGB of
												// Gold for looks

		// lifeDisplay Player
		String lifePL = String.format("%2d%5s%-15s", gameStats[0], "", "Leben Player");

		g.drawString(lifePL, (int) (hud.getX() + ((Hearthstone.BREITE < 1920) ? 18 : 25)),
				(int) (hud.getY() + hud.getHeight() - ((Hearthstone.BREITE < 1920) ? 70 : 85)));

		// manaDisplay Player
		String manaPL = String.format("%2d%5s%-15s", gameStats[1], "", "Mana Player");

		g.drawString(manaPL, (int) (hud.getX() + ((Hearthstone.BREITE < 1920) ? 18 : 25)),
